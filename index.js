...
const game = async () => {
    ...
    if (input.number == number) {
        ...
        if (tries <= 0) {
            spinner.error({
                text: chalk.red("You lose! You left 0 tries. The number was " + number + "\n"),
            });
            retry = false;
            await replay();
            return;
...
const main = async () => {
    ...
    console.log(chalk.magenta(`The computer has generated a random number between 1 and ${level[0]} .
You have to guess the number within `) +
        chalk.magenta.bold.italic(tries + " tries. ") +
        chalk.magenta(`If you guess the number before the tries get zero, you win, otherwise you lose\n`));
};
